    ...
    extends AbstractMultiEnvMojo
{
        ...
        throws IOException, NoSuchArchiverException
    {
        try
        {
            File baseDirectory = new File( getSourceDirectory(), includes );
            File theOriginalFile = new File( baseDirectory, "first.properties" );
            Archiver gzipArchiver = manager.getArchiver( "gzip" );

            gzipArchiver.addFile( theOriginalFile, "first.properties.gz" );

            gzipArchiver.setDestFile( new File( baseDirectory, "first.properties.gz" ) );
